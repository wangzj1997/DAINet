...
class Trainer():
    ...
    def train(self):
        ...
        if 1 <= epoch <= 20 and self.args.load == '.':
        ...
        with torch.no_grad():
            ...
            for idx_scale in scale_list:
                ...
                for idx_img, (lr, hr, filename, _) in tqdm(enumerate(self.loader_test),total=len(self.loader_test)):
                    ...
                    if not no_eval:
                        psnr, ssim, nmse = utility.calc_psnr(idx_img,
                        ...
                        eval_ssim.append(ssim)

                if scale == scale2:
                    logger('[{} x{}]\tNMSE: {:.4f}±{:.4f} PSNR: {:.4f}±{:.4f} SSIM: {:.4f}±{:.4f}'.format(
                        self.args.data_test,
                        scale,
                ...
                eval_psnr_avg.append(np.mean(eval_psnr))
